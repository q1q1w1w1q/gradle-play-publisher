...
import com.github.triplet.gradle.play.tasks.internal.commit
...
open class PublishListing @Inject constructor(
    ...
    fun publishListing(changes: InputChanges) {
        ...
        publisher.commit(ext, variant.applicationId, editId, savedEditId)
    }

    private fun processDetails(executor: WorkerExecutor, changes: InputChanges, editId: String) {
        val changedDetails = changes.getFileChanges(detailFiles)
    ...
    private fun processListings(executor: WorkerExecutor, changes: InputChanges, editId: String) {
        val changedLocales = changes.getFileChanges(listingFiles).asSequence()
                ...
                .filter { it.isDirectChildOf(LISTINGS_PATH) }
                .distinct()
                .filter { it.exists() }
                .toList()
    ...
    private class DetailsUploader @Inject constructor(
        ...
        override fun run() {
            ...
            val details = AppDetails().apply {
                fun AppDetail.read() = File(p.resDir, fileName).orNull()?.readProcessed(maxLength)
        ...
        data class Params(val resDir: File) : Serializable
    }
    ...
    private class ListingUploader @Inject constructor(
        ...
        override fun run() {
            ...
            val listing = Listing().apply {
                fun ListingDetail.read() =
                        File(p.listingDir, fileName).orNull()?.readProcessed(maxLength)
    ...
    private class MediaUploader @Inject constructor(
        ...
        override fun run() {
            ...
            val files = p.imageDir.listFiles()
                    ?.sorted()
                    ?.map { FileContent(MIME_TYPE_IMAGE, it) } ?: return

            check(files.all {
                val isValidType = ImageFileFilter.accept(it.file)
                if (!isValidType) System.err.println("Invalid file type: ${it.file.name}")
                isValidType
            ...
            for (file in files) {
                println("Uploading $locale listing graphic for type '$typeName': ${file.file.name}")
                edits.images().upload(appId, editId, locale, typeName, file).execute()
            }
        }
